...
Finally we want to compare the in sample error and the out of sample error.
...
inSampleError
```
The in out of bag estimated sample error which is the estimated out of sample error based on training with repeated cross validation is `r format(OOB_error*100, digits=2, nsmall=2)`% whereas the in sample error is `r format(inSampleError*100, digits=2, nsmall=2)`% .
