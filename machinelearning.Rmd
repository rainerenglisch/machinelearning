...
Here let's print a summary of our model fit with help of a consusion matrix.
...
### Compare in sample error and out of sample error
Finally we want to compare the in sample error and the out of sample error.  Both errors need to be derived from accuracy variables  stored within the results variable within the prediction model.
...
cvoutOfSampleError
```
The in out of bag estimated sample error which is the estimated out of sample error based on training with repeated cross validation is `r format(cvoutOfSampleError*100, digits=2, nsmall=2)`% whereas the in sample error is `r format(inSampleError*100, digits=2, nsmall=2)`% .
